...
function getDayName(date) {
  ...
  const date2 = date1.toLocaleDateString('EN-US', options);
  return date2;
}
...
function getNextFriday(date) {
  ...
  if (result.getDay() !== dayOfWeek) {
    result.setDate(result.getDate() + ((dayOfWeek + 7 - result.getDay()) % 7));
  } else {
    result.setDate(
      result.getDate() + (((dayOfWeek + 7 - result.getDay()) % 7) + 7)
    );
  }
  return result;
}
...
function getCountDaysInMonth(month, year) {
  const result = 33 - new Date(Date.UTC(year, month - 1, 33)).getDate();
  return result;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const result = Math.ceil((b - a + 1) / (1000 * 3600 * 24));
  return result;
}
